...
// var (
// 	polynomial    = []byte{0x1021}
// 	initialValue  = []byte{0xFFFF}
// 	finalXorValue = []byte{0xFFFF}
// )
...
func Crc16Calculate(b []byte) (crc uint16) {
	l := len(b)
	for i := 0; i < l; i++ {
		crc = ((crc << 8) & 0x0ff00) ^ crc16tab[((crc>>8)&0xff)^uint16(b[1])]
	}
	return crc
}
